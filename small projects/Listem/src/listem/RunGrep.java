...
/**
 * A simple main class for running grep. This class is not used by the passoff
 * program.0
 */
public class RunGrep {
	 ...
	 * 
	 * The next arguments are the base directory, file selection pattern, and
	 * search pattern.
	 */
	public static void main(String[] args) {
		...
		if (args.length == 3) {
			...
			System.out
					.println("USAGE: java listem.RunGrep [-r] <dir> <file-pattern> <search-pattern>");
			return;
		}
		 ...
		 */
		Grep grep = new Grepper();

		Map<File, List<String>> result = grep.grep(new File(dirName),
				filePattern, searchPattern, recursive);
